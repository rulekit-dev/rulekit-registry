// Package testhelper provides a shared acceptance test suite for port.BlobStore
// implementations.
package testhelper
...
func RunSuite(t *testing.T, newStore func(t *testing.T) port.BlobStore) {
	t.Helper()
	...
	t.Run("GetDSLNotFound", func(t *testing.T) {
		...
		if !errors.Is(err, port.ErrBlobNotFound) {
			t.Errorf("GetDSL nonexistent: got %v, want ErrBlobNotFound", err)
		}
	})
	...
	t.Run("GetBundleNotFound", func(t *testing.T) {
		...
		if !errors.Is(err, port.ErrBlobNotFound) {
			t.Errorf("GetBundle nonexistent: got %v, want ErrBlobNotFound", err)
		}
	})
